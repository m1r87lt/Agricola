...
Farmyard::PersonalSupply* Farmyard::personal_supply() const {
	...
	auto log = Location::as_method<decltype(result)>(nullptr, "personal_supply",
			"");

	return result = dynamic_cast<PersonalSupply*>(operator [](0));
}
Farmyard::Space* Farmyard::space(short unsigned row,
		short unsigned column) const {
	Space* result = nullptr;
	auto log = Location::as_method<decltype(result)>(nullptr, "space", "",
			base::Variable<decltype(row)>("row", "", row),
			base::Variable<decltype(column)>("column", "", column));

	if (row > 0 && row < 4 && column > 0 && column < 6)
		result = dynamic_cast<decltype(result)>(operator [](
				(row - 1) * 5 + column));
	else {
		std::string message =
...
Farmyard::FenceSpace* Farmyard::fence(short unsigned row, short unsigned column,
		bool vertical, bool advanced) const {
	FenceSpace* result = nullptr;
	auto log = Location::as_method<decltype(result)>(nullptr, "fence", "",
			base::Variable<decltype(row)>("row", "", row),
			base::Variable<decltype(column)>("column", "", column),
			base::Variable<decltype(vertical)>("vertical", "", vertical),
			base::Variable<decltype(advanced)>("advanced", "", advanced));

	if (row > 0 && row < 4 && column > 0 && column < 6)
		result = dynamic_cast<decltype(result)>(Location::operator [](
...
Farmyard::Row Farmyard::operator [](short unsigned row) const {
	auto log = Location::as_binary<Row>(nullptr, "[]",
			base::Variable<decltype(row)>("row", "", row), "");

	return Row(row, const_cast<Farmyard*>(this), &log);
}

Farmyard::Farmyard(const Player& player, const Log* caller) :
		Location(&player, std::map<std::string, std::string>(), &caller, "",
				true, "", player.gives_variable("player")), Owned(player,
				&caller) {
	auto log = Location::as_constructor(caller, "", typeid(Farmyard), "",
			player.gives_variable("player"));

	Location::inserts_back(
	C_FARMYARD_SPACE, std::unique_ptr<Object>(new Space(&log)), &log);
	Location::inserts_back(C_PERSONAL_SUPPLY,
			std::unique_ptr<Object>(new PersonalSupply(&log)), &log);
	for (size_t r = 1; r < 4; ++r)
		for (size_t c = 1; c < 6; ++c)
			Location::inserts_back(
			C_FARMYARD_SPACE, std::unique_ptr<Object>(new Space(&log)), &log);
	Location::removes(0, &log);
	for (size_t s = 1; s < 39; ++s)
		Location::inserts_back(C_FARMYARD_FENCE_SPACE,
				std::unique_ptr<Object>(new Space(&log)), &log);
}

//Farmyard::PersonalSupply
Farmyard::PersonalSupply::PersonalSupply(Farmyard* owner, const Log* caller) :
		Location(owner, std::map<std::string, std::string>(), caller,
				"Farmyard", false, "",
				base::Variable<decltype(owner)>("owner", "", owner)) {
}

//Farmyard::Space
Farmyard::Space::Space(Farmyard* owner, const Log* caller) :
		Location(owner, std::map<std::string, std::string>(), caller,
				"Farmyard", false, "",
				base::Variable<decltype(owner)>("owner", "", owner)) {
}
...
//Farmyard::FenceSpace
Farmyard::FenceSpace::FenceSpace(Farmyard* owner, const Log* caller) :
		Location(owner, std::map<std::string, std::string>(), caller,
				"Farmyard", false, "",
				base::Variable<decltype(owner)>("owner", "", owner)) {
}
...
//Farmyard::Row
Farmyard::Space* Farmyard::Row::operator [](short unsigned column) {
	Space* result = nullptr;
	auto log = as_binary<Space*>(nullptr, "[]",
			base::Variable<short unsigned>("column", "", column), "");

	return log.returns(owner->space(row, column));
}

Farmyard::Row::Row(short unsigned row, Farmyard* owner, const Log* caller) :
		Log(caller, "", true, "", base::Variable<decltype(row)>("row", "", row),
				base::Variable<decltype(owner)>("owner", "", owner)) {
	if (row < 1 || row > 3) {
...
//Board
Board* Board::construct(const Log* caller) {
	auto log = as_function<Board*>(caller, "", typeid(Board), "costruct", "");

	return log.returns(new Board(&log));
}

Board::Board(const Log* caller) :
		Location(nullptr, std::map<std::string, std::string>(), caller, "",
				false, "") {
	as_constructor(this, "", typeid(Board), "");
}
...
std::vector<std::string> MajorImprovements::gives_them() const {
	...
	return log.returns(result, [](const std::vector<std::string> vector) {
		std::string string = "{";
...
MajorImprovements* MajorImprovements::construct(const Log* caller) {
	auto log = as_function<MajorImprovements*>(caller, "",
			typeid(MajorImprovements), "construct", "");

	return log.returns(new MajorImprovements(&log));
}
...
std::string Data::transcode(const Data& instance) {
	return instance;
}
Data Data::interpret(std::string symbol) {
	Data result;
	auto log = base::Log::as_function<decltype(result)>(nullptr, "",
			typeid(Data), "interpret", "",
			base::Variable<decltype(symbol)>("symbol", "std", symbol));
	auto player = symbol.find("player", 0);
	...
	if (player == std::string::npos)
		...
		try {
			...
			result.quantity = std::stoi(symbol, &index);

		} catch (std::exception& e) {
			log.notes("WARNING what(): " + e.what());
		}
	else
		result.player = 0;
...
Data::Data(int quantity, std::string label, unsigned player, std::string note,
	...
	this->player = player;
	this->note = note;
	std::clog << track.tracker() << "Data::Data(int quantity=" << quantity
			<< ", std::string label=" << label << ", unsigned player=" << player
			<< ", std::string note=\"" << note << "\")" << std::endl;
}
...
void Cover::field(std::string variable, size_t position, std::string value,
		unsigned derived) {
	if (variable == "label") {
		label = value;
...
Cover::Cover(std::string name, std::string caption, std::string type,
	...
	std::clog << track.tracker() << "Cover::Cover(std::string name=\"" << name
			...
			<< "\", std::string type=\"" << type << "\")=" << this << std::endl;
}
...
void Face::field(std::string variable, size_t position, std::string value,
		unsigned derived) {
	...
	if (variable == "pr") {
		...
		if (value == "insert" && position <= pr.size()) {
			...
			pr.emplace(pr.begin() + position, quantity, label, player, note,
					base::Log());
		} else if (value == "erase" && position < pr.size())
			pr.erase(pr.begin() + position);
		else {
			result << "void Face{" << this << "}.field(std::string variable=\""
		...
		if (value == "insert" && position <= c.size()) {
			command >> quantity >> label >> player;
			std::getline(command, note);
			c.emplace(c.begin() + position, quantity, label, player, note,
					base::Log());
...
char Face::edition() const {
	std::clog << base::Log().tracker() << "char Face{" << this << "}.edition()="
			<< e << std::endl;
...
Face::Face(std::vector<Data> prerequisite, std::string name,
	...
	log << track.tracker()
			<< "Face::Face(std::initializer_list< Data > prerequisite="
...
std::string Action::what() const {
	...
	return typeid(*this).name();
		...
		std::string Action::field(std::string variable,
				unsigned derived) const {
		...
		Action::Action(std::string label, Data collection, std::string caption,
				base::Log track) :
				Location(nullptr, track), Executable(label, track), collection(
						collection) {
			this->caption = caption;
			std::clog << track.tracker()
					<< "Action::Action(std::string label=\"" << label
					<< "\", Data collection=" << (std::string) collection
					<< ", std::string caption=\"" << caption << "\")=" << this
					<< std::endl;
		}
...
//Effect
		...
		std::string Effect::field(std::string variable,
				unsigned derived) const {
			if (variable == "caption")
				return caption;
			else
		...
		Effect::Effect(std::string label, std::string caption, base::Log track) :
			...
			std::clog << track.tracker() << "Action::Action(std::string label="
					<< label << ", std::string caption=" << caption << ")="
					<< this << std::endl;
		}
...
//Card
		...
		std::tuple<std::vector<std::unique_ptr<Executable>>, std::vector<Data>,
				std::vector<Data>> Card::face(
				std::map<std::string, std::string> db, base::Log track) {
			...
			buffer.clear();
			for (auto e = 0;
					db.find((buffer = "effect" + std::to_string(e) + "/label"))
							!= db.end(); ++e)
				effects.emplace_back(
						new Effect(db[buffer],
								db["effect" + std::to_string(e) + "/caption"],
								track));
			for (auto a = 0;
					db.find((buffer = "action" + std::to_string(a) + "/label"))
							!= db.end(); ++a) {
				Data data(track);

				if (db.find("action" + std::to_string(a) + "/Data/label")
						!= db.end()) {
					data.quantity =
							std::stoi(
									db["action" + std::to_string(a)
											+ "/Data/quantity"]);
					data.label =
							db["action" + std::to_string(a) + "/Data/label"];
					data.player = std::stoul(
							db["action" + std::to_string(a) + "/Data/player"]);
					data.note = db["action" + std::to_string(a) + "/Data/note"];
				}
				effects.emplace_back(
						new ::Action(db[buffer], data,
								db["action" + std::to_string(a) + "/caption"],
								track));
			}
			for (auto p = 0;
					db.find(
							(buffer = "prerequisite" + std::to_string(p)
									+ "/label")) != db.end(); ++p) {
				prerequisites.emplace_back(track);
				prerequisites.back().quantity = std::stoi(
						db["prerequisite" + std::to_string(p) + "/quantity"]);
				prerequisites.back().label = db["prerequisite"
						+ std::to_string(p) + "/label"];
				prerequisites.back().player = std::stoul(
						db["prerequisite" + std::to_string(p) + "/player"]);
				prerequisites.back().note = db["prerequisite"
						+ std::to_string(p) + "/note"];
			}
			for (auto c = 0;
					db.find((buffer = "cost" + std::to_string(c) + "/label"))
							!= db.end(); ++c) {
				costs.emplace_back(track);
				costs.back().quantity = std::stoi(
						db["cost" + std::to_string(c) + "/quantity"]);
				costs.back().label = db["cost" + std::to_string(c) + "/label"];
				costs.back().player = std::stoul(
						db["cost" + std::to_string(c) + "/player"]);
				costs.back().note = db["cost" + std::to_string(c) + "/note"];
			}
			std::clog << track() << "}={ " << list_unique_ptrs(effects) << " "
					<< list(prerequisites) << " " << list(costs) << " }"
					<< std::endl;
		...
		std::string Card::what() const {
			...
			return typeid(*this).name();
				...
				std::string Card::field(std::string variable,
						unsigned derived) const {
					if (variable == "type")
						return type;
					else if (variable == "c")
				...
				Color Card::color() const {
					std::clog << base::Log().tracker() << "std::string Card{"
							<< this << "}.color()=" << colorNames.at(c)
							<< std::endl;
				...
				std::unique_ptr<Card> Card::occupation(unsigned number,
						base::Log track) {
					std::tuple<std::vector<std::unique_ptr<Executable>>,
							std::vector<Data>, std::vector<Data>> parameters;
					std::map < std::string, std::string > db;
					std::unique_ptr < Card > result;

					std::clog << track.tracker() << "std::unique_ptr<Card> "
							"Card::occupation(unsigned number=" << number
							<< ") {" << std::endl;
					db = base::datafile(std::to_string(number), OCCUPATIONS_DB,
							track);
				...
				std::unique_ptr<Card> Card::improvement(unsigned number,
						base::Log track) {
					std::unique_ptr < Card > result;
					std::tuple<std::vector<std::unique_ptr<Executable>>,
							std::vector<Data>, std::vector<Data>> parameters;
					...
					std::clog << track.tracker()
							<< "std::unique_ptr< Card > Card::occupation(unsigned number="
							<< number << ") {" << std::endl;
					db = base::datafile(std::to_string(number), IMPROVEMENTS_DB,
							track);
					parameters = face(db, track);
					if ((major = db["major"] == "true"))
						result.reset(
								new Card(Improvement::majorType, Color::Crimson,
										std::unique_ptr < Location
												> (new Cover(
														Improvement::majorType,
														Improvement::majorCaption,
														Improvement::majorCover,
														track)),
										std::unique_ptr < Location
												> (new Improvement(major,
														std::stoi(db["vp"]),
														db["oven"] == "true",
														db["bp"] == "true",
														db["kitchen"] == "true",
						...
						result.reset(
								new Card(Improvement::minorType, Color::Orange,
										std::unique_ptr < Location
												> (new Cover(
														Improvement::minorType,
														Improvement::minorCaption,
														Improvement::minorCover,
														track)),
										std::unique_ptr < Location
												> (new Improvement(major,
														std::stoi(db["vp"]),
					...
					if (label == EX_R1_SOW_BAKEBREAD) {
						...
						actions.emplace_back(
								new ::Action(label, Data(track),
										"{t}Sow\n#^(G)(G)->(G)^^(V)->(V)^#{t}"
												"and/or{t}Bake bread{t}",
										track));
						action.reset(
								new Action(0, false, "{R}Stage 1{R}",
										std::move(actions), nullptr, track));
						...
						cover.reset(new Round(1, 1, 4, track));
						actions.emplace_back(
								new ::Action(label, Data(track), "{t}Fences{t}"
						...
						actions.emplace_back(
								new ::Action(label,
										Data(1, "Stone", 0, "", track),
										"1 Stone#(S)#", track));
						action.reset(
								new Card::Action(0, false, "{R}Stage 2{R}",
										std::move(actions), nullptr, track));
						...
						actions.emplace_back(
								new ::Action(label, Data(track),
										...
										track));
						action.reset(
								new Action(0, false, "{R}Stage 2{R}",
										std::move(actions), nullptr, track));
						...
						actions.emplace_back(
								new ::Action(label, Data(track),
										"{t}Take 1 Vegetable{t}"
												"and place it in your personal supply{t}#+(V)#{t}",
										track));
						action.reset(
								new Action(0, false, "{R}Stage 3{R}",
										std::move(actions), nullptr, track));
					} else if (label == EX_R3_WILDBOAR) {
						cover.reset(new Round(3, 8, 9, track));
						actions.emplace_back(
								new ::Action(label,
										Data(1, "Wild Boar", 0, "", track),
										"1 Wild boar\n#[B]#", track));
						action.reset(
								new Action(0, false, "{R}Stage 3{R}",
										std::move(actions), nullptr, track));
						...
						actions.emplace_back(
								new ::Action(label,
										...
										"1 Stone\n#(S)#", track));
						action.reset(
								new Action(0, false, "{R}Stage 4{R}",
										std::move(actions), nullptr, track));
					} else if (label == EX_R4_CATTLE) {
						cover.reset(new Round(4, 10, 11, track));
						actions.emplace_back(
								new ::Action(label,
										Data(1, "Cattle", 0, "", track),
										"1 Cattle\n#[C]#", track));
						action.reset(
								new Action(0, false, "{R}Stage 4{R}",
										std::move(actions), nullptr, track));
						...
						actions.emplace_back(
								new ::Action(label, Data(track),
										...
										track));
						action.reset(
								new Action(0, false, "{R}Stage 5{R}",
										std::move(actions), nullptr, track));
					} else if (label == EX_R5_FAMILYGROWTH) {
						cover.reset(new Round(5, 12, 13, track));
						actions.emplace_back(
								new ::Action(label, Data(track),
										"{t}Family growth{t}even without a room",
										track));
						action.reset(
								new Action(0, false, "{R}Stage 5{R}",
										std::move(actions), nullptr, track));
						...
						actions.emplace_back(
								new ::Action(label, Data(track),
										"After{t}Renovation{t}#1(R)#plus"
												"{l}#1(C)x#room - Clay Hut\n"
												"#1(S)x#room - Stone House{l}"
												"also{t}Fences{t}1 Wood per Fence",
										track));
						action.reset(
								new Action(0, false, "{R}Stage 6{R}",
										std::move(actions), nullptr, track));
					} else {
						std::string message =
								"= ERROR invalid argument: unknown label.";
				...
				std::unique_ptr<Card> Card::action(short unsigned players,
					...
					std::clog << track.tracker()
							...
							<< ", short unsigned family_players="
							<< familyPlayers
							<< ", std::vector< std::tuple< std::string, std::vector< Data >,"
									" std::string > > family_actions="
							<< fa.str() << ") {" << std::endl;
					for (auto ac = actions.begin(); ac != Ac; ++ac)
						actioning.emplace_back(
								new ::Action(std::get < 0 > (*ac),
										std::get < 1 > (*ac),
										std::get < 2 > (*ac), track));
					for (auto fac = actions.begin(); fac != FAc; ++fac)
						familyActioning.emplace_back(
								new ::Action(std::get < 0 > (*fac),
										std::get < 1 > (*fac),
										std::get < 2 > (*fac), track));
					result.reset(
							new Card(Action::type, Color::Green,
									std::unique_ptr < Location
											> (new Action(familyPlayers, true,
													"",
													std::move(familyActioning),
													nullptr, track)),
									std::unique_ptr < Location
											> (new Action(players, false, "",
													std::move(actioning),
													nullptr, track)), nullptr,
									track));
					std::clog << track() << "}" << result.get() << std::endl;
				...
				Card::Card(std::string type, Color color,
						std::unique_ptr<Location>&& cover,
						std::unique_ptr<Location>&& face, Location* position,
						base::Log track) :
						game::Card(std::move(cover), std::move(face), position,
								track) {
					this->type = type;
					c = color;
					std::clog << track.tracker()
							<< "Card::Card(std::string type=\"" << type
...
//Card::Occupation
				...
				std::string Card::Occupation::what() const {
					std::clog << base::Log().tracker()
							<< "std::string Card::Occupation{" << this
							<< "}.what()=" << typeid(*this).name() << std::endl;
					...
					return typeid(*this).name();
						...
						base::Object* Card::Occupation::enter(
								std::string content, size_t number,
								unsigned derived) const {
							return Face::enter(content, number, derived);
						}
						std::string Card::Occupation::field(
								std::string variable, unsigned derived) const {
							if (variable == "p")
						...
						void Card::Occupation::field(std::string variable,
								size_t position, std::string value,
								unsigned derived) {
							if (variable == "p") {
								p = std::stoul(value);
								modification =
						...
						short unsigned Card::Occupation::players() const {
							std::clog << base::Log().tracker()
									<< "short unsigned Card::Occupation{"
						...
						Card::Occupation::Occupation(short unsigned players,
								...
								std::vector<std::unique_ptr<Executable>> events,
								std::vector<Data> prerequisites,
								std::string label, std::vector<Data> costs,
								unsigned number, char edition,
								std::string caption, Location* position,
								base::Log track) :
								Face(prerequisites, label, costs, number,
										edition, caption, std::move(events),
										position, track) {
							bp = bonus_points;
							p = players;
							std::clog << track.tracker()
									<< "Card::Occupation::Occupation(short unsigned players="
									<< players << ", bool bonus_points="
									<< bonus_points
									<< ", std::vector< std::unique_ptr< Executable > > events="
									<< list(executables())
									...
									<< "\", std::initializer_list< Data > costs="
									<< list(costs) << ", unsigned number="
									<< number << ", char edition='" << edition
									<< "', std::string caption=\"" << caption
...
//Card::Improvement
						const std::string Card::Improvement::minorType =
								"Minor Improvement";
						const std::string Card::Improvement::majorType =
								"Major Improvement";
						const std::string Card::Improvement::minorCaption = "";
						const std::string Card::Improvement::majorCaption = "";
						const std::string Card::Improvement::majorCover =
								"MinorImprovement_cover";
						const std::string Card::Improvement::minorCover =
								"MajorImprovement_cover";

						std::string Card::Improvement::what() const {
							std::clog << base::Log().tracker()
									<< "std::string Card::Improvement{" << this
									<< "}.what()=" << typeid(*this).name()
									<< std::endl;
							...
							return typeid(*this).name();
								...
								std::string Card::Improvement::description() const {
									std::clog << base::Log().tracker()
											<< "std::string Card::Improvement{"
											<< this << "}.description()="
											<< caption << std::endl;

									return caption;
								}
								base::Object* Card::Improvement::enter(
										std::string content, size_t number,
										unsigned derived) const {
									return Face::enter(content, number, derived);
								}
								std::string Card::Improvement::field(
										std::string variable,
										unsigned derived) const {
									if (variable == "vp")
								...
								void Card::Improvement::field(
										std::string variable, size_t position,
										std::string value, unsigned derived) {
									if (variable == "vp") {
										vp = std::stoi(value);
										modification =
										...
										Face::field(variable, position, value,
												derived);
								}

								int Card::Improvement::victory_points() const {
									std::clog << base::Log().tracker()
											<< "int Card::Improvement{" << this
											<< "}.victory_points()=" << vp
											<< std::endl;
								...
								Card::Improvement::Improvement(bool major,
										int vp, bool oven, bool bp,
										bool kitchen,
										std::vector<std::unique_ptr<Executable>> events,
										std::vector<Data> prerequisites,
										std::string label,
										...
										std::string caption, Location* position,
										base::Log track) :
										Face(prerequisites, label, costs,
												number, edition, caption,
...
//Card::Round
								const std::string Card::Round::type =
										"Round card";

								unsigned Card::Round::stage() const {
									std::clog << base::Log().tracker()
											<< "unsigned Card::Round{" << this
											<< "}.stage()=" << staging
											<< std::endl;
								...
								std::pair<unsigned, unsigned> Card::Round::round() const {
									std::clog << base::Log().tracker()
											<< "std::pair< unsigned, unsigned > Card::Round{"
											<< this << "}.round()=(" << min
											<< "; " << max << std::endl;
								...
								base::Object* Card::Round::enter(
										std::string content, size_t number,
										unsigned derived) const {
									return Cover::enter(content, number,
											derived);
								}
								std::string Card::Round::field(
										std::string variable,
										unsigned derived) const {
									if (variable == "staging")
								...
								void Card::Round::field(std::string variable,
										size_t position, std::string value,
										unsigned derived) {
									if (variable == "staging") {
										...
										Cover::field(variable, position, value,
												derived);
								}

								Card::Round::Round(unsigned stage, unsigned min,
										unsigned max, base::Log track) :
									...
									this->max = max;
									std::clog << track.tracker()
											<< "Card::Round::Round(unsigned stage="
											<< stage << ", unsigned min=" << min
											<< ", unsigned max=" << max << ")="
											<< this << std::endl;
								}

//Card::Action
								const std::string Card::Action::type =
										"Action card";

								short unsigned Card::Action::players() const {
									std::clog << base::Log().tracker()
											<< "short unsigned Card::Action{"
											<< this << "}.players()=" << p
											<< std::endl;
								...
								std::string Card::Action::what() const {
									...
									return typeid(*this).name();
										...
										base::Object* Card::Action::enter(
												std::string content,
												size_t number,
												unsigned derived) const {
											return Location::enter(content,
													number, derived);
										}
										std::string Card::Action::field(
												std::string variable,
												unsigned derived) const {
											if (variable == "p")
												return std::to_string(p);
										...
										void Card::Action::field(
											...
											if (variable == "p") {
												...
												Location::field(variable,
														position, value,
														derived);
										}

										Card::Action::Action(
												short unsigned players,
												bool family,
												std::string caption,
												std::vector<
											...
											std::clog << track.tracker()
													<< "Card::Action::Action(short unsigned players="
													<< players
													<< ", bool family="
													<< family
													<< ", std::string caption=\""
...
//Card::Begging
										const std::string Card::Begging::type =
												"Begging card";

										std::string Card::Begging::name() const {
											std::clog << base::Log().tracker()
													<< "std::string Card::Begging{"
													<< this << "}.name()="
													<< type << std::endl;
										...
										std::string Card::Begging::description() const {
											std::string result =
													"During the Feeding phase of the Harvest,"
										...
										base::Object* Card::Begging::enter(
												std::string content,
												size_t number,
												unsigned derived) const {
											return Location::enter(content,
										...
										std::string Card::Begging::field(
												std::string variable,
												unsigned derived) const {
											return Location::field(variable,
													derived);
										}
										void Card::Begging::field(
												std::string variable,
												size_t position,
												std::string value,
												unsigned derived) {
											Location::field(variable, position,
													value, derived);
										}

										Card::Begging::Begging(
												Location* position,
												base::Log track) :
												Location(position, track) {
											std::clog << track.tracker()
													<< "Card::Begging::Begging(Location* position="
													<< position << ")=" << this
													<< std::endl;
										}

//Wooden
										const std::map<Wooden::Shape,
												std::string> Wooden::shapeNames =
												{
														{ Wooden::Shape::disc,
																"disc" },
														{ Wooden::Shape::house,
														...
														{ Wooden::Shape::cube,
																"cube" },
														{
																Wooden::Shape::cylinder,
																"cylinder" } };
										const std::string Wooden::woodType =
												"Wood";
										const std::string Wooden::clayType =
												"Clay";
										const std::string Wooden::reedType =
												"Reed";
										const std::string Wooden::stoneType =
												"Stone";
										const std::string Wooden::grainType =
												"Grain";
										const std::string Wooden::vegetableType =
												"Vegetable";
										const std::string Wooden::sheepType =
												"Sheep";
										const std::string Wooden::wildBoarType =
												"Wild Boar";
										const std::string Wooden::cattleType =
												"Cattle";
										const std::string Wooden::familyMemberType =
												"Family Member";
										const std::string Wooden::stableType =
												"Stable";
										const std::string Wooden::fenceType =
												"Fence";
										const std::string Wooden::firstPlayerType =
												"First Player";
										...
										base::Object* Wooden::enter(
												std::string content,
												size_t number,
												unsigned derived) const {
											return Object::enter(content,
													number, derived);
										}
										std::string Wooden::field(
												std::string variable,
												unsigned derived) const {
											if (variable == "type")
												return type;
												...
												return Owned::field(variable,
														derived);
											else
												return Object::field(variable,
														derived);
										}
										void Wooden::field(std::string variable,
												size_t position,
												std::string value,
												unsigned derived) {
											if (variable == "type") {
										...
										std::map<Wooden::Shape, std::string>::const_iterator Wooden::find_shape(
												std::string shape) {
										...
										std::unique_ptr<Wooden> Wooden::wood(
												base::Log track) {
										...
										std::unique_ptr<Wooden> Wooden::clay(
												base::Log track) {
											std::unique_ptr<Wooden> result;
										...
										std::unique_ptr<Wooden> Wooden::reed(
												base::Log track) {
											std::unique_ptr<Wooden> result;
										...
										std::unique_ptr<Wooden> Wooden::stone(
												base::Log track) {
											std::unique_ptr<Wooden> result;
										...
										std::unique_ptr<Wooden> Wooden::grain(
												base::Log track) {
											std::unique_ptr<Wooden> result;
										...
										std::unique_ptr<Wooden> Wooden::vegetable(
												base::Log track) {
											std::unique_ptr<Wooden> result;
										...
										std::unique_ptr<Wooden> Wooden::sheep(
												base::Log track) {
											std::unique_ptr<Wooden> result;
										...
										std::unique_ptr<Wooden> Wooden::wildBoar(
												base::Log track) {
											std::unique_ptr<Wooden> result;
										...
										std::unique_ptr<Wooden> Wooden::cattle(
												base::Log track) {
											std::unique_ptr<Wooden> result;
										...
										std::unique_ptr<Wooden> Wooden::first_player(
												base::Log track) {
											std::unique_ptr<Wooden> result;
										...
										Wooden::Wooden(short unsigned owner,
												std::string type, Shape shape,
												Color color,
												base::Location* position,
												base::Log track) :
...
//Tile
										const std::string Tile::woodenHutType =
												"Wooden Hut";
										const std::string Tile::clayHutType =
												"Clay Hut";
										const std::string Tile::stoneHouseType =
												"Stone House";
										const std::string Tile::fieldType =
												"Field";
										...
										void Tile::field(std::string variable,
												size_t position,
												std::string value,
												unsigned derived) {
											if (variable == "type") {
										...
										std::unique_ptr<Tile> Tile::wooden_hut(
												base::Log track) {
										...
										std::unique_ptr<Tile> Tile::clay_hut(
												base::Log track) {
											std::unique_ptr<Tile> result;
										...
										std::unique_ptr<Tile> Tile::stone_house(
												base::Log track) {
											std::unique_ptr<Tile> result;
										...
										std::unique_ptr<Tile> Tile::field(
												base::Log track) {
											std::unique_ptr<Tile> result;
...
//Token
												const std::string Token::guestType =
														"Guest";
												const std::string Token::tokenType =
														"token";
