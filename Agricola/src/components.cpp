...
Farmyard::PersonalSupply& Farmyard::personal_supply() const {
	auto log = Location::as_method<PersonalSupply&>(nullptr, "personal_supply",
			"");

	return *log.returns(dynamic_cast<PersonalSupply*>(Location::operator [](0)));
}
Farmyard::Space& Farmyard::space(short unsigned row,
		short unsigned column) const {
	auto log = Location::as_method<Space&>(nullptr, "space", "",
...
Farmyard::FenceSpace& Farmyard::fence(short unsigned row, short unsigned column,
	...
	return *log.returns(
			dynamic_cast<FenceSpace*>(Location::operator [](
					11 * row + column + (5 + advanced) * vertical
							+ 11 * advanced + 4)));
}
...
Farmyard::Farmyard(const Player& player, const Log* caller) :
		Location(const_cast<Player*>(&player),
				std::map<std::string, std::string>(), caller, "", true, "",
				player.gives_variable("player")), Owned(player, caller) {
	auto log = Location::as_constructor(caller, "", typeid(Farmyard), "",
			player.gives_variable("player"));

	Location::inserts_back(
	C_FARMYARD_SPACE, std::unique_ptr<Object>(new Space(this, &log)), &log);
	Location::inserts_back(C_PERSONAL_SUPPLY,
			std::unique_ptr<Object>(new PersonalSupply(this, &log)), &log);
	for (size_t r = 1; r < 4; ++r)
		for (size_t c = 1; c < 6; ++c)
			Location::inserts_back(
			C_FARMYARD_SPACE, std::unique_ptr<Object>(new Space(this, &log)),
					&log);
	Location::removes(0, &log);
	for (size_t s = 1; s < 39; ++s)
		Location::inserts_back(C_FARMYARD_FENCE_SPACE,
				std::unique_ptr<Object>(new Space(this, &log)), &log);
}
