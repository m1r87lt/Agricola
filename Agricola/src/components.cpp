...
Farmyard::Space* Farmyard::space(short unsigned row,
	...
	if (row > 0 && row < 4 && column > 0 && column < 6)
		result = dynamic_cast<Space*>(operator []((row - 1) * 5 + column));
	else {
		std::string message =
...
Farmyard::Row Farmyard::operator [](short unsigned row) const {
	auto log = Location::as_method<Row>(nullptr, "[]", "");
...
std::string Gameboard::what() const {
	...
	return typeid(*this).name();
		...
		std::string Gameboard::field(std::string variable,
				unsigned derived) const {
		...
		Gameboard::Gameboard(base::Log track) :
			...
			std::clog << track.tracker() << "Gameboard::Gameboard()=" << this
					<< std::endl;
		}
...
//MajorImprovementBoard
		std::string MajorImprovementBoard::what() const {
			std::clog << base::Log().tracker()
					<< "std::string MajorImprovementBoard{" << this
					<< "}.what()=" << typeid(*this).name() << std::endl;

			return typeid(*this).name();
				}
				base::Object* MajorImprovementBoard::enter(std::string content,
				...
				void MajorImprovementBoard::field(std::string variable,
						size_t position, std::string value, unsigned derived) {
					Location::field(variable, position, value, derived);
				}
				...
				MajorImprovementBoard::MajorImprovementBoard(base::Log track) :
					...
					std::clog << track.tracker()
							<< "MajorImprovementBoard::MajorImprovementBoard()="
							<< this << std::endl;
				}
...
//Data
				...
				Data::Data(int quantity, std::string label, unsigned player,
						std::string note, base::Log track) {
...
//Cover
				...
				std::string Cover::field(std::string variable,
						unsigned derived) const {
					if (variable == "label")
						return label;
				...
				Cover::Cover(std::string name, std::string caption,
						std::string type, base::Log track) :
						Location(nullptr, track) {
					label = name;
					text = caption;
					this->type = type;
					std::clog << track.tracker()
							<< "Cover::Cover(std::string name=\"" << name
...
//Face
				...
				std::string Face::field(std::string variable,
						unsigned derived) const {
					if (variable == "pr")
						return list(pr);
					else if (variable == "l")
				...
				void Face::field(std::string variable, size_t position,
					...
					if (variable == "pr") {
						...
						if (value == "insert" && position <= pr.size()) {
							...
							pr.emplace(pr.begin() + position, quantity, label,
									player, note, base::Log());
						...
						if (value == "insert" && position <= c.size()) {
							command >> quantity >> label >> player;
							std::getline(command, note);
							c.emplace(c.begin() + position, quantity, label,
									player, note, base::Log());
						...
						if (value == "insert" && position <= events.size()) {
							command >> event;
							events.emplace(events.begin() + position,
									(Executable*) event);
				...
				const std::vector<Data>& Face::prerequisites() const {
					std::clog << base::Log().tracker()
							<< "const std::vector< Data >& Face{" << this
							<< "}.prerequisites()=" << list(pr) << std::endl;

					return pr;
				}
				const std::string& Face::name() const {
					std::clog << base::Log().tracker()
							<< "const std::string& Face{" << this << "}.name()="
							<< l << std::endl;
				...
				const std::vector<Data>& Face::costs() const {
					std::clog << base::Log().tracker()
							<< "const std::vector< Data >& Face{" << this
							<< "}.costs()=" << list(c) << std::endl;

					return c;
				}
				unsigned Face::number() const {
					std::clog << base::Log().tracker() << "unsigned Face{"
							<< this << "}.number()=" << n << std::endl;
				...
				std::vector<std::string> Face::executables() const {
					...
					std::clog << track.tracker()
							<< "std::vector< std::string > Face{" << this
				...
				Executable& Face::operator ()(size_t e) {
					...
					log << track.tracker() << "Executable& Face{" << this
							<< "}[" << e << "]";
					std::clog << log.str() << " {" << std::endl;
					if (e < events.size()) {
						modification = std::chrono::system_clock::to_time_t(
								std::chrono::system_clock::now());
						std::clog << track() << "}=" << (std::string) *events[e]
								<< std::endl;
						...
						message
								<< "= ERROR invalid argument: e >= event number ="
								<< events.size() << ".";
						std::clog << track() << "}=" << message.str()
								<< std::endl;
				...
				Face::Face(std::vector<Data> prerequisite, std::string name,
					...
					log << track.tracker()
							...
							<< list(prerequisite) << ", std::string name=\""
							<< name
							<< "\", std::initializer_list< Data > costs="
							<< list(costs) << ", unsigned number=" << number
							<< ", char edition='" << edition
							<< "', std::string caption=\"" << caption
							<< "\", std::vector< std::unique_ptr< Executable > > events="
							<< Executable::list(events)
				...
				Face::~Face() {
					if (n && base::running())
						cards.erase(n);
					std::clog << base::Log().tracker() << "Face::~Face(){"
							<< this << "}" << std::endl;
				}

//Action
				const Data& Action::collect() const {
					std::clog << base::Log().tracker() << "const Data& Action{"
							<< this << "}.collect()="
							<< (std::string) collection << std::endl;

					return collection;
				}

				std::string Action::what() const {
					std::clog << base::Log().tracker() << "std::string Action{"
							<< this << "}.what()=" << typeid(*this).name()
							<< std::endl;
					...
					return typeid(*this).name();
						...
						std::string Action::description() const {
							std::clog << base::Log().tracker()
									<< "std::string Action{" << this
									<< "}.description()=" << caption
									<< std::endl;

							return caption;
						}
						base::Object* Action::enter(std::string content,
								size_t number, unsigned derived) const {
							return Location::enter(content, number, derived);
						}
						...
						void Action::field(std::string variable,
							...
							if (variable == "collection") {
								command >> quantity >> label >> player;
								std::getline(command, note);
								collection = Data(quantity, label, player, note,
										base::Log());
								modification =
								...
								modification =
									...
									|| variable == "name")
								Executable::field(variable, position, value,
										derived);
							else
								Location::field(variable, position, value,
										derived);
						}

						Action::Action(std::string label, Data collection,
								std::string caption, base::Log track) :
								Location(nullptr, track), Executable(label,
...
//Effect
						...
						void Effect::field(std::string variable,
								size_t position, std::string value,
								unsigned derived) {
							if (variable == "caption")
								caption = value;
						...
						Effect::Effect(std::string label, std::string caption,
								base::Log track) :
								Executable(label, track) {
							std::clog << track.tracker()
									<< "Action::Action(std::string label="
									<< label << ", std::string caption="
									<< caption << ")=" << this << std::endl;
						}
...
//Card
						...
						std::tuple<std::vector<std::unique_ptr<Executable>>,
								std::vector<Data>, std::vector<Data>> Card::face(
							...
							for (auto a = 0;
								...
								if (db.find(
										"action" + std::to_string(a)
												+ "/Data/label") != db.end()) {
									data.quantity = std::stoi(
											db["action" + std::to_string(a)
													+ "/Data/quantity"]);
									data.label = db["action" + std::to_string(a)
											+ "/Data/label"];
									data.player = std::stoul(
											db["action" + std::to_string(a)
													+ "/Data/player"]);
									data.note = db["action" + std::to_string(a)
											+ "/Data/note"];
								}
								effects.emplace_back(
							...
							for (auto p = 0;
								...
								prerequisites.back().quantity = std::stoi(
										db["prerequisite" + std::to_string(p)
												+ "/quantity"]);
							...
							for (auto c = 0;
									db.find(
											(buffer = "cost" + std::to_string(c)
													+ "/label")) != db.end();
									++c) {
								costs.emplace_back(track);
								costs.back().quantity = std::stoi(
										db["cost" + std::to_string(c)
												+ "/quantity"]);
								costs.back().label = db["cost"
										+ std::to_string(c) + "/label"];
								costs.back().player = std::stoul(
										db["cost" + std::to_string(c)
												+ "/player"]);
								costs.back().note = db["cost"
										+ std::to_string(c) + "/note"];
							}
							std::clog << track() << "}={ "
									<< list_unique_ptrs(effects) << " "
									<< list(prerequisites) << " " << list(costs)
									<< " }" << std::endl;

							return std::make_tuple(std::move(effects),
									prerequisites, costs);
						}

						std::string Card::what() const {
							std::clog << base::Log().tracker()
									<< "std::string Card{" << this
									<< "}.what()=" << typeid(*this).name()
									<< std::endl;

							return typeid(*this).name();
								}
								base::Object* Card::enter(std::string content,
										size_t number, unsigned derived) const {
									return game::Card::enter(content, number,
											derived);
								}
								std::string Card::field(std::string variable,
									...
									if (variable == "type")
										...
										return game::Card::field(variable,
												derived);
								}
								void Card::field(std::string variable,
										size_t position, std::string value,
										unsigned derived) {
									if (variable == "type") {
										type = value;
										...
										game::Card::field(variable, position,
												value, derived);
								}
								...
								Color Card::color() const {
									std::clog << base::Log().tracker()
											<< "std::string Card{" << this
											<< "}.color()=" << colorNames.at(c)
											<< std::endl;

									return c;
								}

								std::unique_ptr<Card> Card::occupation(
										unsigned number, base::Log track) {
									std::tuple<
											std::vector<
									...
									std::clog << track.tracker()
											<< "std::unique_ptr<Card> "
													"Card::occupation(unsigned number="
											<< number << ") {" << std::endl;
									db = base::datafile(std::to_string(number),
											OCCUPATIONS_DB, track);
									parameters = face(db, track);
									if (!db.empty())
										result.reset(
												new Card(Occupation::type,
														Color::Yellow,
														std::unique_ptr
																< Location
																> (new Cover(
														...
														std::unique_ptr
																...
																> (new Occupation(
																		...
																		track)),
														nullptr, track));
									std::clog << track() << "}=" << result.get()
											<< std::endl;

									return std::move(result);
								}
								std::unique_ptr<Card> Card::improvement(
										unsigned number, base::Log track) {
									std::unique_ptr < Card > result;
									std::tuple<
											std::vector<
									...
									std::clog << track.tracker()
											...
											<< number << ") {" << std::endl;
									db = base::datafile(std::to_string(number),
											IMPROVEMENTS_DB, track);
									parameters = face(db, track);
									if ((major = db["major"] == "true"))
										result.reset(
												new Card(Improvement::majorType,
														Color::Crimson,
														std::unique_ptr
																< Location
																> (new Cover(
										...
										result.reset(
												new Card(Improvement::minorType,
														Color::Orange,
														std::unique_ptr
																< Location
																> (new Cover(
								...
								std::unique_ptr<Card> Card::round(
										std::string label, base::Log track) {
									...
									if (label == EX_R1_SOW_BAKEBREAD) {
										...
										actions.emplace_back(
												new ::Action(label,
														...
														"1 Stone#(S)#", track));
										action.reset(
												new Card::Action(0, false,
														"{R}Stage 2{R}",
														std::move(actions),
														nullptr, track));
									} else if (label
											== EX_R2_RENOVATION_IMPROVEMENT) {
										cover.reset(new Round(2, 5, 7, track));
										actions.emplace_back(
												new ::Action(label, Data(track),
										...
										action.reset(
											...
											== EX_R2_FAMILYGROWTH_MINORIMPROVEMENT) {
										cover.reset(new Round(2, 5, 7, track));
										actions.emplace_back(
												new ::Action(label, Data(track),
									...
									result.reset(
											new Card(Round::type, Color::Blue,
													std::move(cover),
													std::move(action), nullptr,
													track));
									std::clog << track() << "}=" << result.get()
											<< std::endl;
								...
								std::unique_ptr<Card> Card::action(
									...
									for (auto ac = actions.begin(); ac != Ac;
											++ac)
										a << ", (" << std::get < 0
												> (*ac) << "; "
														<< (std::string) std::get
									...
									a.str(
											actions.size() ?
													...
													"{");
									a << " }";
									for (auto fac = actions.begin(); fac != FAc;
											++fac)
										fa << ", (" << std::get < 0
												> (*fac) << "; "
														<< (std::string) std::get
								...
								std::unique_ptr<Card> Card::begging(
										base::Log track) {
									std::unique_ptr < Card > result;
								...
								Card::Card(std::string type, Color color,
										...
										std::unique_ptr<Location>&& face,
										Location* position, base::Log track) :
										game::Card(std::move(cover),
												std::move(face), position,
												track) {
									this->type = type;
									c = color;
									std::clog << track.tracker()
...
//Card::Occupation
								const std::string Card::Occupation::type =
										"Occupation";
								const std::string Card::Occupation::caption = "";
								const std::string Card::Occupation::cover =
										"Occupation_cover";

								std::string Card::Occupation::what() const {
									std::clog << base::Log().tracker()
											<< "std::string Card::Occupation{"
											<< this << "}.what()="
											<< typeid(*this).name()
											<< std::endl;
									...
									return typeid(*this).name();
										...
										base::Object* Card::Occupation::enter(
												...
												size_t number,
												unsigned derived) const {
											return Face::enter(content, number,
													derived);
										}
										std::string Card::Occupation::field(
											...
											if (variable == "p")
												...
												return Face::field(variable,
														derived);
										}
										void Card::Occupation::field(
												std::string variable,
												size_t position,
												std::string value,
												unsigned derived) {
										...
										Card::Occupation::Occupation(
												...
												Location* position,
												base::Log track) :
												Face(prerequisites, label,
														costs, number, edition,
														caption,
														std::move(events),
														position, track) {
											bp = bonus_points;
...
//Wooden
														...
														std::string Wooden::field(
															...
															if (variable
																...
																return type;
																	...
																	== "c")
																return colorNames.at(
																		c);
															else if (variable
																	== "s")
																return shapeNames.at(
																		s);
															else if (variable
																	== "owner")
																return Owned::field(
														...
														std::map<Wooden::Shape,
																std::string>::const_iterator Wooden::find_shape(
																std::string shape) {
															auto s =
																	shapeNames.begin();
															auto S =
